...
def main():
    if len(sys.argv) != 2:
        print("Usage: python scripts/urban_compute_auto_features_v2.py <config.json>")
        sys.exit(1)
    ...
    for _, r in df.iterrows():
        ...
        # vollständig featurebasiertes Routing
        shadow_like = (
            (
                not np.isnan(dark_frac) and not np.isnan(mean_drop) and
                dark_frac >= thr["shadow_dark_frac"] and
                mean_drop >= thr["shadow_mean_drop"]
            )
            or
            (
                not np.isnan(inside_std) and not np.isnan(mean_drop) and
                inside_std >= thr["shadow_inside_std"] and
                mean_drop >= thr["shadow_mean_drop_relaxed"]
            )
        )

        topology_like = (
            feat_hull_gap >= thr["topology_hull_gap_frac"]
            or feat_compactness <= thr["topology_compactness_max"]
            or (
                not np.isnan(edge_density)
                and edge_density >= thr["topology_edge_density_high"]
                and uncertainty_level in ["medium", "high"]
            )
        )
    ...
    out_csv = out_dir / "urban_auto_features_v2.csv"
    ...
    print(out[[
        ...
        "dark_frac", "inside_std", "mean_drop_to_ring",
        "edge_density", "compactness", "hull_gap_frac", "route_match"
    ]])
    print()
