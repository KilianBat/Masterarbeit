...
# alte Datei löschen, damit keine alten Layer drin bleiben
if out_gpkg.exists():
    out_gpkg.unlink()
if out_csv.exists():
    out_csv.unlink()
...
for _, r in gdf.iterrows():
    ...
    rows_final.append({
        "id": obj_id,
        "decision": decision,
        "final_source": final_source,   # osm / sam2 / osm_review
        "sam_score": sam_score,
        ...
        "orig_wkt": orig.wkt,
        "pred_wkt": pred_wkt,
        "geometry": final_geom,         # <-- DAS ist jetzt das echte Update-Produkt
    })
...
print("  layers: update_product, original_osm, predicted_sam2")
