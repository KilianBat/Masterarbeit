...
def main():
    if len(sys.argv) != 2:
        print("Usage: python scripts/geo_make_chips_from_current_subset.py <config.json>")
        sys.exit(1)
    ...
    subset_path = ROOT / "data" / "processed" / subset_name / "current_subset.gpkg"
    ...
    for _, row in gdf.iterrows():
        ...
        rows.append({
            "id": chip_id,
            "current_osm_id": row.get("current_osm_id", None),
            "chip_path": str(chip_path),
            "current_wkt": geom.wkt,
